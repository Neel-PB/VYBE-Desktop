...
import { SETTINGS_COLOR_SWITCH_ON } from '../vybeSettingsDesignTokens.js';
import { IStorageService, StorageScope, StorageTarget } from '../../../../../platform/storage/common/storage.js';
import { DisposableStore } from '../../../../../base/common/lifecycle.js';
// Local constant when vybeChat/vybeDiffDecorations is not present
const STORAGE_KEY_ENABLE_DIFF_DECORATIONS = 'vybe.diffDecorations.enabled';
...
export function renderGeneralTab(
	...
	const accountSectionList = accountSection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const accountSubSection = DOM.append(accountSectionList, DOM.$('.vybe-settings-sub-section'));
	...
	const prefsSectionList = prefsSection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const prefsSubSection = DOM.append(prefsSectionList, DOM.$('.vybe-settings-sub-section'));
	...
	const resetSubSection = DOM.append(prefsSectionList, DOM.$('.vybe-settings-sub-section'));
	...
	const notifSectionList = notifSection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const notifSubSection = DOM.append(notifSectionList, DOM.$('.vybe-settings-sub-section'));
	...
	const aiEditsSectionList = aiEditsSection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const aiEditsSubSection = DOM.append(aiEditsSectionList, DOM.$('.vybe-settings-sub-section'));
	...
	if (diffDecorationsSwitch) {
		const updateToggleVisual = (checked: boolean) => {
			...
			if (bgFill && knob) {
				diffDecorationsSwitch.style.background = checked ? SETTINGS_COLOR_SWITCH_ON : 'rgba(128, 128, 128, 0.3)';
	...
	const privacySectionList = privacySection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const privacySubSection = DOM.append(privacySectionList, DOM.$('.vybe-settings-sub-section'));
