...
export function renderCloudAgentsTab(parent: HTMLElement): void {
	...
	integrationsSection.id = 'vybe-settings-integrations';
	const integrationsSectionList = integrationsSection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const integrationsSubSection = DOM.append(integrationsSectionList, DOM.$('.vybe-settings-sub-section'));
	...
	const slackTrailing = slackCell.querySelector('.vybe-settings-cell-trailing-items') as HTMLElement;
	if (slackTrailing) {
		...
		button.className = 'flex flex-nowrap items-center justify-center gap-[4px] px-[6px] rounded cursor-pointer whitespace-nowrap shrink-0 vybe-button';
	...
	// Add description
	const sectionHeader = workspaceSection.querySelector('.vybe-settings-section-header') as HTMLElement;
	if (sectionHeader) {
		const leadingItems = sectionHeader.querySelector('.vybe-settings-section-header-leading-items') as HTMLElement;
		if (leadingItems) {
			// Check if description already exists, if not create it
			let description = leadingItems.querySelector('.vybe-settings-section-header-description') as HTMLElement;
			if (!description) {
				description = DOM.append(leadingItems, DOM.$('.vybe-settings-section-header-description'));
			}
	...
	const workspaceSectionList = workspaceSection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const workspaceSubSection = DOM.append(workspaceSectionList, DOM.$('.vybe-settings-sub-section'));

	// Create sub-section-list with background (like Manage Settings and Connect Slack)
	const workspaceSubSectionList = DOM.append(workspaceSubSection, DOM.$('.vybe-settings-sub-section-list'));
	...
	const configCell = DOM.append(workspaceSubSectionList, DOM.$('.vybe-settings-cell'));
