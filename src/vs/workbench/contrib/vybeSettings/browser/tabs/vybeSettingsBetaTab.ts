...
export function renderBetaTab(parent: HTMLElement): void {
	...
	const betaSectionList = betaSection.querySelector('.vybe-settings-section-list') as HTMLElement;

	// Update Access sub-section
	const updateSubSection = DOM.append(betaSectionList, DOM.$('.vybe-settings-sub-section'));
	const updateSubSectionList = DOM.append(updateSubSection, DOM.$('.vybe-settings-sub-section-list'));
	...
	// Update Access cell
	const updateCell = DOM.append(updateSubSectionList, DOM.$('.vybe-settings-cell.vybe-settings-cell-align-top'));
	...
	const updateLeading = DOM.append(updateCell, DOM.$('.vybe-settings-cell-leading-items'));
	updateLeading.style.cssText = 'display: flex; flex-direction: column; gap: 1px; flex: 1;';

	const updateLabel = DOM.append(updateLeading, DOM.$('p.vybe-settings-cell-label'));
	...
	const updateDesc = DOM.append(updateLeading, DOM.$('p.vybe-settings-cell-description'));
	...
	const updateTrailing = DOM.append(updateCell, DOM.$('.vybe-settings-cell-trailing-items'));
	...
	updateDropdownChevron.style.cssText = 'color: var(--vscode-descriptionForeground); font-size: 14px;';

	// Beta features sub-section
	const featuresSubSection = DOM.append(betaSectionList, DOM.$('.vybe-settings-sub-section'));
	const featuresSubSectionList = DOM.append(featuresSubSection, DOM.$('.vybe-settings-sub-section-list'));
	...
	const devSectionList = devSection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const devSubSection = DOM.append(devSectionList, DOM.$('.vybe-settings-sub-section'));
	const devSubSectionList = DOM.append(devSubSection, DOM.$('.vybe-settings-sub-section-list'));
