...
import { IVybeLLMModelService, ModelHoverPopup, type VybeModelRow } from '../vybeSettingsModelsStubs.js';
...
export function renderModelsTab(
	...
	const modelsSectionList = modelsSection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const modelsSubSection = DOM.append(modelsSectionList, DOM.$('.vybe-settings-sub-section'));

	// Create sub-section-list for models
	const modelsSubSectionList = DOM.append(modelsSubSection, DOM.$('.vybe-settings-sub-section-list'));
	...
	const searchCell = DOM.append(searchContainer, DOM.$('.vybe-settings-cell.vybe-settings-cell-align-center'));
		...
		try {
			...
			if (modelService) {
				...
				if (searchQuery.trim()) {
					...
					filteredModels = filteredModels.filter((m: VybeModelRow) =>
				...
				if (filteredModels.length === 0) {
					const emptyCell = DOM.append(modelsList, DOM.$('.vybe-settings-cell'));
					...
					for (const model of filteredModels) {
							const modelCell = DOM.append(modelsList, DOM.$('.vybe-settings-cell'));
							...
							const toggleContainer = DOM.append(modelCell, DOM.$('.vybe-settings-cell-switch-container'));
							...
							switchOuter.style.cssText = `
								...
								overflow: hidden;
								background: ${modelEnabled ? SETTINGS_COLOR_SWITCH_ON : 'rgba(128, 128, 128, 0.3)'};
							`;
							...
							bgFill.style.cssText = `
								...
								opacity: ${modelEnabled ? '1' : '0'};
								width: ${modelEnabled ? '100%' : '0%'};
							...
							knob.style.cssText = `
								...
								left: ${modelEnabled ? 'calc(100% - 16px)' : '2px'};
							`;

							// Toggle click handler
							let isEnabled = modelEnabled;
							disposables.add(addDisposableListener(toggleContainer, EventType.CLICK, () => {
								isEnabled = !isEnabled;
								modelService.setModelEnabled?.(model.id ?? '', isEnabled);

								// Update visual state
								switchOuter.style.background = isEnabled ? SETTINGS_COLOR_SWITCH_ON : 'rgba(128, 128, 128, 0.3)';
