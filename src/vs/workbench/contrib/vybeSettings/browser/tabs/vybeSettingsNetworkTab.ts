...
export function renderNetworkTab(parent: HTMLElement): void {
	...
	const networkSectionList = networkSection.querySelector('.vybe-settings-section-list') as HTMLElement;

	// HTTP Compatibility Mode sub-section
	const httpSubSection = DOM.append(networkSectionList, DOM.$('.vybe-settings-sub-section'));
	const httpSubSectionList = DOM.append(httpSubSection, DOM.$('.vybe-settings-sub-section-list'));
	...
	const httpCell = DOM.append(httpSubSectionList, DOM.$('.vybe-settings-cell.vybe-settings-cell-align-top'));
	...
	const httpLeading = DOM.append(httpCell, DOM.$('.vybe-settings-cell-leading-items'));
	httpLeading.style.cssText = 'display: flex; flex-direction: column; gap: 1px; flex: 1;';

	const httpLabel = DOM.append(httpLeading, DOM.$('p.vybe-settings-cell-label'));
	...
	const httpDesc = DOM.append(httpLeading, DOM.$('p.vybe-settings-cell-description'));
	...
	const httpTrailing = DOM.append(httpCell, DOM.$('.vybe-settings-cell-trailing-items'));
	...
	httpDropdownChevron.style.cssText = 'color: var(--vscode-descriptionForeground); font-size: 14px;';

	// Network Diagnostics sub-section
	const diagnosticsSubSection = DOM.append(networkSectionList, DOM.$('.vybe-settings-sub-section'));
	const diagnosticsSubSectionList = DOM.append(diagnosticsSubSection, DOM.$('.vybe-settings-sub-section-list'));
	...
	const diagnosticsCell = DOM.append(diagnosticsSubSectionList, DOM.$('.vybe-settings-cell.vybe-settings-cell-align-top'));
	...
	const diagnosticsLeading = DOM.append(diagnosticsCell, DOM.$('.vybe-settings-cell-leading-items'));
	diagnosticsLeading.style.cssText = 'display: flex; flex-direction: column; gap: 1px; flex: 1;';

	const diagnosticsLabel = DOM.append(diagnosticsLeading, DOM.$('p.vybe-settings-cell-label'));
	...
	const diagnosticsDesc = DOM.append(diagnosticsLeading, DOM.$('div.vybe-settings-cell-description'));
	...
	const diagnosticsTrailing = DOM.append(diagnosticsCell, DOM.$('.vybe-settings-cell-trailing-items'));
	...
	const diagnosticsButton = DOM.append(diagnosticsButtonContainer, DOM.$('div.vybe-button.vybe-button-tertiary.vybe-button-tertiary-clickable.vybe-button-small'));
