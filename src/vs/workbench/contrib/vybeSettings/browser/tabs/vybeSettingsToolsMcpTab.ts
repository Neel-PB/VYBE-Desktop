...
function createMcpServerItem(parent: HTMLElement, server: McpServerItem, hasDivider: boolean = false): HTMLElement {
	...
	if (hasDivider) {
		const divider = DOM.append(container, DOM.$('.vybe-settings-cell-divider'));
	...
	toolsButton.className = 'flex flex-nowrap items-center justify-center gap-[4px] px-[6px] rounded cursor-pointer whitespace-nowrap shrink-0 vybe-text-button';
	...
	// Toggle switch
	const switchContainer = DOM.append(controls, DOM.$('.vybe-settings-cell-switch-container'));
	...
	switchOuter.style.cssText = `
		...
		overflow: hidden;
		background: ${server.checked ? SETTINGS_COLOR_SWITCH_ON : 'rgba(128, 128, 128, 0.3)'};
	`;
...
export function renderToolsMcpTab(parent: HTMLElement): void {
	...
	const browserSectionList = browserSection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const browserSubSection = DOM.append(browserSectionList, DOM.$('.vybe-settings-sub-section'));

	// Create sub-section-list with background
	const browserSubSectionList = DOM.append(browserSubSection, DOM.$('.vybe-settings-sub-section-list'));
	...
	browserDropdownChevron.style.cssText = 'color: var(--vscode-descriptionForeground); font-size: 14px;';
	...
	const mcpSectionList = mcpSection.querySelector('.vybe-settings-section-list') as HTMLElement;
	const mcpSubSection = DOM.append(mcpSectionList, DOM.$('.vybe-settings-sub-section'));

	// Create sub-section-list with background
	const mcpSubSectionList = DOM.append(mcpSubSection, DOM.$('.vybe-settings-sub-section-list'));
	...
	const newServerDivider = DOM.append(newServerContainer, DOM.$('.vybe-settings-cell-divider'));
